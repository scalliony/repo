...
use super::{
    gradient, math,
    math::{F, P},
    NoiseFn, PermutationTable, Seedable,
};
use std::ops::Add;

const STRETCH_CONSTANT_2D: F = -0.211_324_865_405_187; //(1/sqrt(2+1)-1)/2;
const SQUISH_CONSTANT_2D: F = 0.366_025_403_784_439; //(sqrt(2+1)-1)/2;
const STRETCH_CONSTANT_3D: F = -1.0 / 6.0; //(1/Math.sqrt(3+1)-1)/3;
const SQUISH_CONSTANT_3D: F = 1.0 / 3.0; //(Math.sqrt(3+1)-1)/3;
const STRETCH_CONSTANT_4D: F = -0.138_196_601_125_011; //(Math.sqrt(4+1)-1)/4;
const SQUISH_CONSTANT_4D: F = 0.309_016_994_374_947; //(Math.sqrt(4+1)-1)/4;

const NORM_CONSTANT_2D: F = 1.0 / 14.0;
const NORM_CONSTANT_3D: F = 1.0 / 14.0;
const NORM_CONSTANT_4D: F = 1.0 / 6.869_909_007_095_662_5;
...
impl NoiseFn<2> for OpenSimplex {
    fn get(&self, point: P<2>) -> F {
        fn gradient(perm_table: &PermutationTable, vertex: P<2>, pos: P<2>) -> F {
            let attn = 2.0 - math::dot(pos, pos);
            if attn > 0.0 {
                let index = perm_table.get2(math::to_isize(vertex));
                let vec = gradient::get2(index);
                attn.powi(4) * math::dot(pos, vec)
            } else {
                0.0
            }
        }
        ...
        let stretch_offset = math::fold(point, Add::add) * STRETCH_CONSTANT_2D;
        let stretched = math::map(point, |v| v + stretch_offset);

        // Floor to get grid coordinates of rhombus (stretched square) cell origin.
        let stretched_floor = math::map(stretched, F::floor);

        // Skew out to get actual coordinates of rhombus origin. We'll need these later.
        let squish_offset = math::fold(stretched_floor, Add::add) * SQUISH_CONSTANT_2D;
        let skewed_floor = math::map(stretched_floor, |v| v + squish_offset);

        // Compute grid coordinates relative to rhombus origin.
        let rel_coords = math::sub(stretched, stretched_floor);

        // Sum those together to get a value that determines which region we're in.
        let region_sum = math::fold(rel_coords, Add::add);

        // Positions relative to origin point (0, 0).
        let pos0 = math::sub(point, skewed_floor);
        ...
        // Contribution (1, 0)
        vertex = math::add(stretched_floor, [1.0, 0.0]);
        dpos = math::sub(pos0, [t1, t0]);
        value += gradient(&self.perm_table, vertex, dpos);

        // Contribution (0, 1)
        vertex = math::add(stretched_floor, [0.0, 1.0]);
        dpos = math::sub(pos0, [t0, t1]);
        value += gradient(&self.perm_table, vertex, dpos);
        ...
        if region_sum > 1.0 {
            // Contribution (1, 1)
            vertex = math::add(stretched_floor, [1.0, 1.0]);
            // We are moving across the diagonal `/`, so we'll need to add by the
            // squish constant
            dpos = math::sub(pos0, [t2, t2]);
        } else {
            vertex = math::add(stretched_floor, [0.0, 0.0]);
            dpos = math::sub(pos0, [0.0, 0.0]);
        }
...
impl NoiseFn<3> for OpenSimplex {
    fn get(&self, point: [f64; 3]) -> f64 {
        fn gradient(perm_table: &PermutationTable, vertex: [f64; 3], pos: [f64; 3]) -> f64 {
            let attn = 2.0 - math::dot(pos, pos);
            if attn > 0.0 {
                let index = perm_table.get3(math::to_isize(vertex));
        ...
        let stretch_offset = math::fold(point, Add::add) * STRETCH_CONSTANT_3D;
        let stretched = math::map(point, |v| v + stretch_offset);

        // Floor to get simplectic h1.0ycomb coordinates of rhombohedron
        // (stretched cube) super-cell origin.
        let stretched_floor = math::map(stretched, f64::floor);

        // Skew out to get actual coordinates of rhombohedron origin. We'll need
        // these later.
        let squish_offset = math::fold(stretched_floor, Add::add) * SQUISH_CONSTANT_3D;
        let skewed_floor = math::map(stretched_floor, |v| v + squish_offset);
        ...
        if region_sum <= 1.0 {
            ...
            vertex = math::add(stretched_floor, [0.0, 0.0, 0.0]);
            dpos = math::sub(pos0, [0.0, 0.0, 0.0]);
            value += gradient(&self.perm_table, vertex, dpos);

            // Contribution at (1, 0, 0)
            vertex = math::add(stretched_floor, [1.0, 0.0, 0.0]);
            dpos = math::sub(pos0, [t1, t0, t0]);
            value += gradient(&self.perm_table, vertex, dpos);

            // Contribution at (0, 1, 0)
            vertex = math::add(stretched_floor, [0.0, 1.0, 0.0]);
            dpos = math::sub(pos0, [t0, t1, t0]);
            value += gradient(&self.perm_table, vertex, dpos);

            // Contribution at (0, 0, 1)
            vertex = math::add(stretched_floor, [0.0, 0.0, 1.0]);
            dpos = math::sub(pos0, [t0, t0, t1]);
            value += gradient(&self.perm_table, vertex, dpos);
            ...
            vertex = math::add(stretched_floor, [1.0, 1.0, 0.0]);
            dpos = math::sub(pos0, [t1, t1, t0]);
            value += gradient(&self.perm_table, vertex, dpos);

            // Contribution at (1, 0, 1)
            vertex = math::add(stretched_floor, [1.0, 0.0, 1.0]);
            dpos = math::sub(pos0, [t1, t0, t1]);
            value += gradient(&self.perm_table, vertex, dpos);

            // Contribution at (0, 1, 1)
            vertex = math::add(stretched_floor, [0.0, 1.0, 1.0]);
            dpos = math::sub(pos0, [t0, t1, t1]);
            value += gradient(&self.perm_table, vertex, dpos);

            // Contribution at (1, 1, 1)
            vertex = math::add(stretched_floor, [1.0, 1.0, 1.0]);
            dpos = math::sub(pos0, [t2, t2, t2]);
            value += gradient(&self.perm_table, vertex, dpos);
        } else {
            ...
            // Contribution at (1, 1, 0)
            vertex = math::add(stretched_floor, [1.0, 1.0, 0.0]);
            dpos = math::sub(pos0, [t3, t3, t2]);
            value += gradient(&self.perm_table, vertex, dpos);

            // Contribution at (1, 0, 1)
            vertex = math::add(stretched_floor, [1.0, 0.0, 1.0]);
            dpos = math::sub(pos0, [t3, t2, t3]);
            value += gradient(&self.perm_table, vertex, dpos);

            // Contribution at (0, 1, 1)
            vertex = math::add(stretched_floor, [0.0, 1.0, 1.0]);
            dpos = math::sub(pos0, [t2, t3, t3]);
            value += gradient(&self.perm_table, vertex, dpos);
        }
...
impl NoiseFn<4> for OpenSimplex {
    fn get(&self, point: [f64; 4]) -> f64 {
        #[inline(always)]
        fn gradient(perm_table: &PermutationTable, vertex: [f64; 4], pos: [f64; 4]) -> f64 {
            let attn = 2.0 - math::dot(pos, pos);
            if attn > 0.0 {
                let index = perm_table.get4(math::to_isize(vertex));
        ...
        // Place input coordinates on simplectic h1.0ycomb.
        let stretch_offset = math::fold(point, Add::add) * STRETCH_CONSTANT_4D;
        let stretched = math::map(point, |v| v + stretch_offset);
        ...
        let squish_offset = math::fold(stretched_floor, Add::add) * SQUISH_CONSTANT_4D;
        let skewed_floor = math::map(stretched_floor, |v| v + squish_offset);
        ...
        let mut pos0 = math::sub(point, skewed_floor);
        ...
        if region_sum <= 1.0 {
            ...
            let pos1;
            {
                let vertex = math::add(stretched_floor, [1.0, 0.0, 0.0, 0.0]);
                pos1 = math::sub(
                    pos0,
                    [
            ...
            let pos2;
            {
                let vertex = math::add(stretched_floor, [0.0, 1.0, 0.0, 0.0]);
            ...
            let pos3;
            {
                let vertex = math::add(stretched_floor, [0.0, 0.0, 1.0, 0.0]);
            ...
            let pos4;
            {
                let vertex = math::add(stretched_floor, [0.0, 0.0, 0.0, 1.0]);
            ...
            let pos4;
            {
                let vertex = math::add(stretched_floor, [1.0, 1.0, 1.0, 0.0]);
                pos4 = math::sub(
                    pos0,
                    [
            ...
            let pos3;
            {
                let vertex = math::add(stretched_floor, [1.0, 1.0, 0.0, 1.0]);
            ...
            let pos2;
            {
                let vertex = math::add(stretched_floor, [1.0, 0.0, 1.0, 1.0]);
            ...
            let pos1;
            {
                let vertex = math::add(stretched_floor, [0.0, 1.0, 1.0, 1.0]);
            ...
            {
                let vertex = math::add(stretched_floor, [1.0, 1.0, 1.0, 1.0]);
            ...
            let pos5;
            {
                let vertex = math::add(stretched_floor, [1.0, 1.0, 0.0, 0.0]);
                pos5 = [
            ...
            let pos6;
            {
                let vertex = math::add(stretched_floor, [1.0, 0.0, 1.0, 0.0]);
            ...
            let pos7;
            {
                let vertex = math::add(stretched_floor, [1.0, 0.0, 0.0, 1.0]);
            ...
            let pos8;
            {
                let vertex = math::add(stretched_floor, [0.0, 1.0, 1.0, 0.0]);
            ...
            let pos9;
            {
                let vertex = math::add(stretched_floor, [0.0, 1.0, 0.0, 1.0]);
            ...
            let pos10;
            {
                let vertex = math::add(stretched_floor, [0.0, 0.0, 1.0, 1.0]);
